...
export default function Trends() {
  ...
  const loadWeek = useCallback(async () => {
    ...
    const [foodRes, whoopRes, patternRes, userRes] = await Promise.all([
      ...
      supabase.from("whoop_cache").select("date, recovery_score, hrv_rmssd, strain_score, calories_burned_kcal").in("date", dates).order("date", { ascending: true }),
    ...
    setWhoopData(
      dates.map((d) => {
        ...
        return { date: d, recovery_score: row?.recovery_score ?? null, calories_burned_kcal: row?.calories_burned_kcal ?? null };
      })
    );
  ...
  const refMinCal = Math.round(calorieTarget * 0.97);
  const refMaxCal = Math.round(calorieTarget * 1.03);
  const maxCal = Math.max(...calData.map((d) => d.calories), refMaxCal, 1);
  ...
  const refMinY = refMinCal <= maxCal ? CHART_HEIGHT - (refMinCal / maxCal) * CHART_HEIGHT : CHART_HEIGHT + 5;
  const refMaxY = refMaxCal <= maxCal ? CHART_HEIGHT - (refMaxCal / maxCal) * CHART_HEIGHT : CHART_HEIGHT + 5;
  ...
  const exertionData = dates.map((d) => {
    const cal = calData.find((x) => x.date === d);
    const whoop = whoopData.find((x) => x.date === d);
    return { date: d, intake: cal?.calories ?? 0, exertion: whoop?.calories_burned_kcal ?? 0 };
  });
  const maxIntakeExertion = Math.max(...exertionData.flatMap((x) => [x.intake, x.exertion]), 1);
    ...
    >
      ...
      >
        ...
        <View style={{ height: CHART_HEIGHT + 30, position: "relative" }}>
          {/* Ref line min */}
          <View style={{ position: "absolute", left: 0, right: 0, top: refMinY, height: 0, borderStyle: "dashed", borderTopWidth: 1.5, borderColor: "rgba(255,255,255,0.22)" }} />
          <Text style={{ position: "absolute", right: 0, top: refMinY - 6, fontFamily: fontFamily.regular, fontSize: 10, color: colors.textTertiary }}>{refMinCal}</Text>
          {/* Ref line max */}
          <View style={{ position: "absolute", left: 0, right: 0, top: refMaxY, height: 0, borderStyle: "dashed", borderTopWidth: 1.5, borderColor: "rgba(255,255,255,0.22)" }} />
          <Text style={{ position: "absolute", right: 0, top: refMaxY - 6, fontFamily: fontFamily.regular, fontSize: 10, color: colors.textTertiary }}>{refMaxCal}</Text>
          <View style={{ flexDirection: "row", height: CHART_HEIGHT, alignItems: "flex-end", justifyContent: "space-between" }}>
            {calData.map((d) => {
              const isToday = d.date === todayStr;
              const w = isToday ? BAR_WIDTH_TODAY : BAR_WIDTH;
              const barH = Math.max((d.calories / maxCal) * CHART_HEIGHT, d.calories > 0 ? 2 : 0);
              const pct = calorieTarget > 0 ? d.calories / calorieTarget : 0;
              const useGradient = d.calories > 0 && pct >= 0.9;
              return (
                <Pressable key={d.date} style={{ width: w, alignItems: "center", justifyContent: "flex-end" }} onPress={() => setTooltipDate(tooltipDate === d.date ? null : d.date)}>
                  {tooltipDate === d.date && (
                    <View style={{ position: "absolute", bottom: barH + 8, backgroundColor: colors.surfaceL3, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12 }}>
                      <Text style={{ fontFamily: fontFamily.regular, fontSize: 12, color: colors.textPrimary }}>{dayAbbr(d.date)}: {d.calories} cal</Text>
                    </View>
                  )}
                  {useGradient ? (
                    <LinearGradient
                      colors={[colors.accentBlue, colors.purple]}
                      style={{
                        width: w,
                        height: barH,
                        borderTopLeftRadius: 6,
                        borderTopRightRadius: 6,
                        ...(isToday && { shadowColor: colors.accentBlue, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.3, shadowRadius: 12 }),
                      }}
                    />
                  ) : (
                    <View
                      style={{
                        width: w,
                        height: barH,
                        backgroundColor: d.calories > 0 ? calorieBarColor(pct) : "rgba(255,255,255,0.1)",
                        borderTopLeftRadius: 6,
                        borderTopRightRadius: 6,
                        ...(isToday && d.calories > 0 && { shadowColor: colors.accentBlue, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.3, shadowRadius: 12 }),
                      }}
                    />
                  )}
                </Pressable>
        ...
        <View style={{ flexDirection: "row", marginTop: 6, justifyContent: "space-between" }}>
          {calData.map((d) => (
            <View key={d.date} style={{ width: d.date === todayStr ? BAR_WIDTH_TODAY : BAR_WIDTH, alignItems: "center" }}>
      ...
      >
        ...
        <View style={{ flexDirection: "row", height: 60, alignItems: "flex-end", justifyContent: "space-between" }}>
          {calData.map((d) => {
            ...
            const useGradient = d.protein > 0 && pct >= 0.9;
            return (
              <View key={d.date} style={{ width: w, alignItems: "center", justifyContent: "flex-end" }}>
                {useGradient ? (
                  <LinearGradient
                    colors={[...holoGradients.protein]}
                    style={{
                      width: w,
                      height: barH,
                      borderTopLeftRadius: 6,
                      borderTopRightRadius: 6,
                      ...(isToday && { shadowColor: colors.accentGreen, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.3, shadowRadius: 12 }),
                    }}
                  />
                ) : (
                  <View
                    style={{
                      width: w,
                      height: barH,
                      backgroundColor: d.protein > 0 ? proteinBarColor(pct) : "rgba(255,255,255,0.1)",
                      borderTopLeftRadius: 6,
                      borderTopRightRadius: 6,
                      ...(isToday && d.protein > 0 && { shadowColor: colors.accentGreen, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.3, shadowRadius: 12 }),
                    }}
                  />
                )}
              </View>
      ...
      {/* Recovery vs nutrition overlay — card */}
      {hasRecovery && (
        <View style={{ marginTop: 24, backgroundColor: colors.surface, borderRadius: radius.cardLg, padding: 16, borderWidth: 1, borderColor: colors.border, ...shadows.card }}>
      ...
      >
        <Text style={{ fontFamily: fontFamily.regular, fontSize: 11, color: colors.textTertiary, letterSpacing: 1.5, marginBottom: 10 }}>INTAKE vs. EXERTION</Text>
        <View style={{ flexDirection: "row", height: 72, alignItems: "flex-end", justifyContent: "space-between" }}>
          {exertionData.map((d) => (
            <View key={d.date} style={{ flex: 1, alignItems: "center", marginHorizontal: 2 }}>
              <View style={{ flexDirection: "row", height: 60, alignItems: "flex-end", justifyContent: "center", gap: 4 }}>
                <View
                  style={{
                    width: 12,
                    height: Math.max((d.exertion / maxIntakeExertion) * 60, d.exertion > 0 ? 2 : 0),
                ...
                <View
                  style={{
                    width: 12,
                    height: Math.max((d.intake / maxIntakeExertion) * 60, d.intake > 0 ? 2 : 0),
              ...
              <Text style={{ fontFamily: fontFamily.regular, fontSize: 10, color: colors.textTertiary, marginTop: 4 }}>{dayAbbr(d.date)}</Text>
            </View>
          ))}
        </View>
        <Text style={{ fontFamily: fontFamily.regular, fontSize: 12, color: colors.textSecondary, marginTop: 10 }}>
          {(() => {
            const avgDeficit = exertionData.length
              ? exertionData.reduce((s, x) => s + (x.exertion - x.intake), 0) / exertionData.length
              : 0;
            return `Avg ${Math.round(avgDeficit)} cal ${avgDeficit >= 0 ? "deficit" : "surplus"}/day — for recomp, target 200–500 cal deficit.`;
          })()}
        </Text>
      </View>
